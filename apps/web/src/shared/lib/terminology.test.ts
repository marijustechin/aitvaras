...
/** Apps/web/src — the whole user-facing source tree is scanned. */
...
describe("Lithuanian UI terminology", () => {
  ...
  it("uses Vaidmenys on the admin users page", () => {
    const source = readSource("features", "manage-users", "ui", "users-page.tsx");
    expect(source).toContain("Vaidmenys");
    expect(source).not.toMatch(/\bRolės\b/);
  });
  ...
  it("renders no legacy Lithuanian role/status terminology anywhere in the UI", () => {
    const files = ["app", "entities", "features", "widgets", "shared"].flatMap(
      (layer) => collectSourceFiles(join(srcDir, layer)),
    );
